#!/usr/bin/env python3
"""OpenTelemetry tracing configuration for the Supply Chain Backend API."""
...
class NoisySpanFilter(SpanExporter):
    """Custom span exporter that filters out noisy framework spans."""
...
class TracingConfig:
    ...
    def initialize(self, service_name: str = "supply-chain-backend", 
        ...
        try:
            ...
            if jaeger_host:
                logger.info(f"🔗 Adding OTLP exporter for Jaeger at {jaeger_host}:{jaeger_port}")
                ...
                logger.info("✅ OTLP span exporter added for Jaeger")
            else:
                logger.warning("⚠️  No Jaeger host configured - OTLP exporter not added")
                logger.info("💡 Set JAEGER_HOST environment variable to enable Jaeger export")
            ...
            # Instrument HTTPX client and FastAPI
    ...
    def _initialize_noop_tracer(self, service_name: str):
        ...
        try:
            ...
            self._initialized = True
            self.tracer = None
            logger.critical("Tracing completely disabled - backend will continue without tracing")
    ...
    def span(self, name: str, attributes: Optional[Dict[str, Any]] = None, 
        ...
        # Create spans without parent context to avoid errors
...
def initialize_tracing(service_name: str = "supply-chain-backend", 
                      jaeger_host: Optional[str] = None,
                      jaeger_port: int = 4317,
                      enable_console_exporter: bool = None):
