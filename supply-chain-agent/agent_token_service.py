...
import aauth
from stable_identity import load_or_create_stable_identity

logger = logging.getLogger(__name__)

_STABLE_ID_DIR = Path(__file__).resolve().parent


def _agent_server_base_from_env() -> str:
    raw = (os.environ.get("AGENT_SERVER_BASE") or "").strip()
    return (raw or "http://127.0.0.1:8765").rstrip("/")


def _agent_name_from_env() -> str:
    return (
        os.getenv("SUPPLY_CHAIN_AGENT_NAME") or "Supply Chain Optimizer Agent"
    ).strip() or "Supply Chain Optimizer Agent"
...
class AgentTokenService:
    ...
    def get_ephemeral_pub_jwk(self) -> dict[str, Any] | None:
        """Current PoP public JWK (matches agent_token cnf.jwk); for /jwks.json if needed."""
        return self._eph_pub_jwk
    ...
    async def startup(self) -> None:
        ...
        base = _agent_server_base_from_env()
    ...
    async def _register_or_poll(self, client: httpx.AsyncClient, base: str) -> None:
        ...
        body_obj = {
            ...
            "agent_name": _agent_name_from_env(),
        }
