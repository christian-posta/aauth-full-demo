...
try:
    ...
    HAS_JWT = False
    logger.warning("⚠️ PyJWT not available - resource token generation will fail")
...
def generate_resource_token(
    ...
    auth_server_url: str,
    scope: str = "supply-chain:optimize",
    ...
    # Get resource's signing keypair
    ...
    # Prepare JWT header
    kid = public_jwk.get("kid", "supply-chain-agent-key-1")
    header = {
        "typ": "resource+jwt",
    ...
    # PyJWT's EdDSA algorithm expects the private key in the format used by cryptography library
    try:
        token = jwt.encode(
            payload,
            private_key,
