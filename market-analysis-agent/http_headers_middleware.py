...
# CANONICAL_AUTHORITY should be set to match what signers will use (e.g., "market-analysis-agent.localhost:3000" or "localhost:9998")
CANONICAL_AUTHORITY = os.getenv("CANONICAL_AUTHORITY", None)
if not CANONICAL_AUTHORITY:
    # Fallback: derive from MARKET_ANALYSIS_AGENT_URL if CANONICAL_AUTHORITY not set
    CANONICAL_AUTHORITY_URL = os.getenv("MARKET_ANALYSIS_AGENT_URL", "http://localhost:9998/")
    _canonical_parsed = urlparse(CANONICAL_AUTHORITY_URL)
    CANONICAL_AUTHORITY = _canonical_parsed.netloc  # e.g., "localhost:9998" or "localhost" (if default port)
    CANONICAL_SCHEME = _canonical_parsed.scheme or "http"
else:
    # CANONICAL_AUTHORITY is set directly (format: "host:port" or just "host")
    # Extract scheme from MARKET_ANALYSIS_AGENT_URL if available, otherwise default to http
    CANONICAL_AUTHORITY_URL = os.getenv("MARKET_ANALYSIS_AGENT_URL", "http://localhost:9998/")
    _canonical_parsed = urlparse(CANONICAL_AUTHORITY_URL)
    CANONICAL_SCHEME = _canonical_parsed.scheme or "http"
...
class HTTPHeadersCaptureMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        if DEBUG:
            ...
            logger.debug(f"🔍 Canonical authority: {CANONICAL_AUTHORITY}")
